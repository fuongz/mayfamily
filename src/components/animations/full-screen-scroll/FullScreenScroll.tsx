  ...
  return (
    <div className="relative w-full h-screen overflow-hidden touch-none">
      <div
        ref={containerRef}
        className={cn(
          "w-full h-full overflow-y-auto snap-y snap-mandatory scroll-smooth overscroll-none",
          className
        )}
      ...
      {showDots && (
        <div
          className={cn(
            "fixed top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 p-2 bg-black/50 rounded-full",
            dotsPosition === "right" ? "right-1" : "left-1",
            dotsClassName
          )}
        >
          {sections.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                index === activeSection
                  ? cn("bg-white scale-125", activeDotClassName)
                  : cn("bg-white/50 hover:bg-white/75", dotClassName)
              )}
